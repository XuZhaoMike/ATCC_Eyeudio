...
class Demo:
    ...
    def _draw_gaze_vector(self, face: Face) -> None:
        ky = 5  # Proportional gain for x movement
        kp = 5  # Proportional gain for y movement
        prev_pitch = 0
        ...
        if self.config.mode == 'MPIIGaze':
            for key in [FacePartsName.REYE, FacePartsName.LEYE]:
                ...
                yaw_f = "{:.2f}".format(yaw)
                pitch_f = "{:.2f}".format(pitch)
                print('Pitch:' + str(pitch_f) + ',  Yaw:' + str(yaw_f))
                # logger.info(f'[{key.name.lower()}] pitch: {pitch:.2f}, yaw: {yaw:.2f}') 
                # mouse.move(round(ky*yaw)+1000, 0, absolute=True, duration=0.1)
        elif self.config.mode in ['MPIIFaceGaze', 'ETH-XGaze']:
            self.visualizer.draw_3d_line(
                face.center, face.center + length * face.gaze_vector)
            pitch, yaw = np.rad2deg(face.vector_to_angle(face.gaze_vector))
            # logger.info(f'[face] pitch: {pitch:.2f}, yaw: {yaw:.2f}')
            # mouse.move(yaw*ky, -(pitch-prev_pitch)*kp-100, absolute=False, duration=0.075) 
            prev_pitch = pitch
        else:
            raise ValueError
